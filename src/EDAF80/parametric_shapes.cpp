	...
	unsigned int const vertical_split_count)
{
	...
	auto y_offset = -4.0f;
	auto z_offset = height / 2;

	int index = 0;
	for (unsigned int x = 0u; x < horizontal_vertices_count; ++x) {
		for (unsigned int z = 0u; z < vertical_vertices_count; ++z) {
			//vertecies
			vertices[index] = glm::vec3((x * width) / horizontal_edges_count - x_offset, y_offset, (z * height) / vertical_edges_count - z_offset);

			// texture coordinates
			texcoords[index] = glm::vec3(static_cast<float>(x) / (static_cast<float>(horizontal_vertices_count)),
				static_cast<float>(z) / (static_cast<float>(vertical_vertices_count)),
				0.0f);
	...
	for (unsigned int i = 0u; i < horizontal_edges_count; ++i)
	{
		for (unsigned int j = 0u; j < vertical_edges_count; ++j)
		{
			index_sets[index] = glm::uvec3(	horizontal_vertices_count * (i + 0u) + (j + 0u),
											...
											index_sets[index] = glm::uvec3(horizontal_vertices_count * (i + 0u) + (j + 1u),
